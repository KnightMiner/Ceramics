...
/**
 * Logic to load models for how a faucet should flow into a block
 */
public class FaucetFluidLoader extends JsonReloadListener {
  ...
  protected void apply(Map<ResourceLocation,JsonElement> map, IResourceManager resourceManager, IProfiler profiler) {
    ...
    if (def == null || !def.isJsonObject()) {
      Ceramics.LOG.warn("Found no default fluid model, this is likely a problem with the resource pack");
      defaultFluid = FaucetFluid.EMPTY;
    } else {
    ...
    for (Entry<ResourceLocation,JsonElement> entry : map.entrySet()) {
      ...
      try {
        ...
        if(block != null && block != Blocks.AIR) {
          ...
          Ceramics.LOG.debug("Skipping loading faucet fluid model '{}' as no coorsponding block exists", location);
        }
      } catch (Exception e) {
        Ceramics.LOG.warn("Exception loading faucet fluid model '{}': {}", location, e.getMessage());
      }
    }
  }
